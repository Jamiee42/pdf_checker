...
## 界面截图
- 主界面
![主界面，包含三个区域：上方输入区（PDF 路径和报告输出路径选择）、中间分析区（开始分析按钮和进度条）、下方输出区（分析简报和导出按钮）](/screenshot/1.png)
- 统计报告
![Excel 报告，列的内容包含文件名、总页数、可编辑/不可编辑/空白页/无法识别的页数、注意（复核提示）](/screenshot/2.png)
